...
namespace sql {
ObDropTableResolver::ObDropTableResolver(ObResolverParams& params) : ObDDLResolver(params)
{}
...
int ObDropTableResolver::resolve(const ParseNode& parse_tree)
{
  ...
  if (OB_ISNULL(session_info_) || OB_ISNULL(allocator_) ||
      (T_DROP_TABLE != parse_tree.type_ && T_DROP_VIEW != parse_tree.type_) || MAX_NODE != parse_tree.num_child_ ||
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ret)) {
      ...
      drop_table_arg.table_type_ = share::schema::USER_VIEW;
    ...
    if (NULL ==
        (tmp_ptr = (ObPlacementHashSet<ObTableItem>*)allocator_->alloc(sizeof(ObPlacementHashSet<ObTableItem>)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      ...
      for (i = 0; OB_SUCC(ret) && i < max_table_num; ++i) {
        table_node = share::is_oracle_mode() ? parse_tree.children_[TABLE_LIST_NODE]
        ...
        if (NULL == table_node) {
          ...
          db_name.reset();
          table_name.reset();
          if (OB_FAIL(resolve_table_relation_node(table_node, table_name, db_name))) {
            ...
            if (OB_FAIL(session_info_->get_name_case_mode(table_item.mode_))) {
              ...
              if (OB_HASH_EXIST == table_item_set->exist_refactored(table_item)) {
                ...
                if (OB_FAIL(schema_checker_->check_table_exists(tenant_id, db_name, table_name, false, is_exists))) {
                  SQL_RESV_LOG(WARN, "failed to check table or view exists", K(db_name), K(table_name), K(ret));
                } else if (!is_exists) {
                  ...
                  } else if (OB_FAIL(schema_checker_->get_simple_table_schema(
                                 tenant_id, db_id, table_name, false, table_view_schema))) {
                    SQL_RESV_LOG(WARN, "failed to get simple table schema", K(db_id), K(table_name), K(ret));
                  } else if (NULL == table_view_schema) {
                    SQL_RESV_LOG(WARN, "table_view_schema is NULL", K(ret), K(db_id), K(table_name));
                  } else if ((T_DROP_TABLE == parse_tree.type_ && table_view_schema->is_view_table()) ||
                             (T_DROP_VIEW == parse_tree.type_ && !table_view_schema->is_view_table())) {
                    ret = OB_TABLE_NOT_EXIST;
                    LOG_USER_ERROR(OB_TABLE_NOT_EXIST, to_cstring(db_name), to_cstring(table_name));
                  } else if (OB_FAIL(schema_checker_->check_ora_ddl_priv(tenant_id,
                                 ...
                                 T_DROP_TABLE == parse_tree.type_ ? stmt::T_DROP_TABLE : stmt::T_DROP_VIEW,
                                 session_info_->get_enable_role_array()))) {
                    if (OB_TABLE_NOT_EXIST == ret) {
                      LOG_USER_ERROR(OB_TABLE_NOT_EXIST, to_cstring(db_name), to_cstring(table_name));
                    }
                    SQL_RESV_LOG(WARN,
                        "failed to check ora ddl priv",
                        K(db_name),
                        K(parse_tree.type_),
                        K(table_name),
