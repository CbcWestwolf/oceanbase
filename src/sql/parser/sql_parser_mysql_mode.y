...
/*Please add the newly added non-reserved keywords to the corresponding rows in alphabetical order, thanks! ! !*/
        ...
        EXTENDED_NOADDR EXTENT_SIZE EXTRACT EXCEPT EXPIRED EXTERNAL EXTERNAL_URL EXTERNAL_PROTOCAL EXTERNAL_FORMAT LINE_DELIMITER FIELD_DELIMITER
...
| EXTERNAL_URL opt_equal_mark STRING_VALUE
{
  (void)($2);
  malloc_non_terminal_node($$, result->malloc_pool_, T_EXTERNAL_URL, 1, $3);
}
| EXTERNAL_PROTOCAL opt_equal_mark STRING_VALUE
{
  (void)($2);
  malloc_non_terminal_node($$, result->malloc_pool_, T_EXTERNAL_PROTOCAL, 1, $3);
}
| EXTERNAL_FORMAT opt_equal_mark STRING_VALUE
{
  (void)($2);
  malloc_non_terminal_node($$, result->malloc_pool_, T_EXTERNAL_FORMAT, 1, $3);
}
| LINE_DELIMITER opt_equal_mark STRING_VALUE
{
  (void)($2);
  malloc_non_terminal_node($$, result->malloc_pool_, T_LINE_DELIMITER, 1, $3);
}
| FIELD_DELIMITER opt_equal_mark STRING_VALUE
{
  (void)($2);
  malloc_non_terminal_node($$, result->malloc_pool_, T_FIELD_DELIMITER, 1, $3);
}
